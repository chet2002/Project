...
void setup_robot(struct Robot *robot){
    ...
    robot->crashed = 0;
    robot->auto_mode = 0;
    robot->right_hugging = 0;
    robot->left_hugging = 0;
...
int checkRobotSensorBackRightAllWalls(struct Robot * robot, struct Wall_collection * head) {
    ...
    {
        ptr = head_store;
        xDir = round(robotCentreX+(ROBOT_WIDTH/2-5)*cos((90+(robot->angle))*PI/180)-((ROBOT_HEIGHT-28)-SENSOR_VISION+sensorSensitivityLength*i)*sin((90+(robot->angle))*PI/180));
        yDir = round(robotCentreY+(ROBOT_WIDTH/2-5)*sin((90+(robot->angle))*PI/180)+((ROBOT_HEIGHT-28)-SENSOR_VISION+sensorSensitivityLength*i)*cos((90+(robot->angle))*PI/180));
        xTL = (int) xDir;
        yTL = (int) yDir;
        hit = 0;
...
int checkRobotSensorBackLeftAllWalls(struct Robot * robot, struct Wall_collection * head) {
    ...
    {
        ptr = head_store;
        xDir = round(robotCentreX+(-ROBOT_WIDTH/2+5)*cos((270+(robot->angle))*PI/180)-((ROBOT_HEIGHT-29)-SENSOR_VISION+sensorSensitivityLength*i)*sin((270+(robot->angle))*PI/180));
        yDir = round(robotCentreY+(-ROBOT_WIDTH/2+5)*sin((270+(robot->angle))*PI/180)+((ROBOT_HEIGHT-29)-SENSOR_VISION+sensorSensitivityLength*i)*cos((270+(robot->angle))*PI/180));
        xTL = (int) xDir;
        yTL = (int) yDir;
        hit = 0;
...
void robotUpdate(struct SDL_Renderer * renderer, struct Robot * robot){
    ...
    */
    ...
    for (i = 0; i < 5; i++)
    {
        xDir = round(robotCentreX+(ROBOT_WIDTH/2-5)*cos((90+(robot->angle))*PI/180)-((ROBOT_HEIGHT-28)-SENSOR_VISION+sensor_sensitivity*i)*sin((90+(robot->angle))*PI/180));
        yDir = round(robotCentreY+(ROBOT_WIDTH/2-5)*sin((90+(robot->angle))*PI/180)+((ROBOT_HEIGHT-28)-SENSOR_VISION+sensor_sensitivity*i)*cos((90+(robot->angle))*PI/180));
        xTL = (int) xDir;
        yTL = (int) yDir;
    ...
    for (i = 0; i < 5; i++)
    {
        xDir = round(robotCentreX+(-ROBOT_WIDTH/2+5)*cos((270+(robot->angle))*PI/180)-((ROBOT_HEIGHT-29)-SENSOR_VISION+sensor_sensitivity*i)*sin((270+(robot->angle))*PI/180));
        yDir = round(robotCentreY+(-ROBOT_WIDTH/2+5)*sin((270+(robot->angle))*PI/180)+((ROBOT_HEIGHT-29)-SENSOR_VISION+sensor_sensitivity*i)*cos((270+(robot->angle))*PI/180));
        xTL = (int) xDir;
        yTL = (int) yDir;
...
void robotMotorMove(struct Robot * robot) {
    ...
    switch(robot->direction){
        case UP :
            ...
            if (robot->currentSpeed > (MAX_ROBOT_SPEED/5))
                robot->currentSpeed = (MAX_ROBOT_SPEED/5); //Capping speed to max speed /5 (so 5)
            break;
        case DOWN :
            robot->currentSpeed -= DEFAULT_SPEED_CHANGE;
            if (robot->currentSpeed < -(MAX_ROBOT_SPEED/5))
...
void robotAutoMotorMove(struct Robot * robot, int front_left_sensor, int front_right_sensor, int back_left_sensor, int back_right_sensor) {
    if (robot->left_hugging == 1 && back_left_sensor == 0){ //If robot WAS hugging left, but no longer hugging, turn LEFT
        robot->direction = LEFT;
        robot->left_hugging = 0;
    }

    if (robot->right_hugging == 1 && back_right_sensor == 0){ //If robot WAS hugging right, but no longer hugging, turn RIGHT
        robot->direction = RIGHT;
        robot->right_hugging = 0;
    }

    else if ((front_left_sensor == 0) && (front_right_sensor == 0) && (back_left_sensor == 0) && (back_right_sensor == 0)) { //If no sensors are detecting anything, move forward. Default when at standstill. Used to get robot moving
        if (robot->currentSpeed<2){
            robot->direction = UP;
        }
    }
    if ((robot->currentSpeed>0) && ((front_left_sensor == 0) && (front_right_sensor == 0) && (back_left_sensor > 0) && (back_right_sensor == 0)) ) { //left hugging wall
        robot->direction = UP;
        robot->left_hugging = 1;
    }
    else if ((robot->currentSpeed>0) && (front_left_sensor == 0) && (front_right_sensor == 0) && (back_left_sensor == 0) && (back_right_sensor > 0) ) { //right hugging wall
        robot->direction = UP;
        robot->right_hugging = 1;
    }
    else if ((robot->currentSpeed>0) && (front_left_sensor == 0) && (front_right_sensor == 0) && (back_left_sensor > 0) && (back_right_sensor > 0) ) { //narrow corridor, implemented to expand upon in event of complex maze being unsolvable with current algorithm
        robot->direction = UP;
    }
    if ((robot->currentSpeed>0) && ((front_left_sensor >= 1) || (front_right_sensor >= 1)) && back_left_sensor == 0 ) { //if both front sensors detect an incoming wall, and the left side is free, default to turn left to avoid incoming wall
        robot->direction = LEFT;
    }
    else if ((robot->currentSpeed>0) && ((front_left_sensor >= 1) || (front_right_sensor >= 1)) && back_right_sensor == 0 ) { //else if both front sensors detect an incoming wall, and the right side is free, turn right to avoid incoming wall
        robot->direction = RIGHT;
    }
    else if ((robot->currentSpeed>0) && (front_left_sensor == 0) && (front_right_sensor == 0) && (back_left_sensor == 0) && (back_right_sensor == 0) ) { //try to find right wall when no sensors are detecting anything.
        robot->direction = FIND_RIGHT;
    }
}

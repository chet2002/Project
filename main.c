...
int main(int argc, char *argv[]) {
    ...
    int front_left_sensor, front_right_sensor, back_left_sensor, back_right_sensor;
    ...
    while(!done){
        ...
        if (!robot.crashed)
         {
            if (robot.auto_mode == 1)
                robotAutoMotorMove(&robot, front_left_sensor, front_right_sensor, back_left_sensor, back_right_sensor);
        ...
        //Otherwise compute sensor information
            ...
            if (back_right_sensor>0)
                printf("Getting close on the back right. Score = %d\n", back_right_sensor);
        }
